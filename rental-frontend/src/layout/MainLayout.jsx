  ...
  return (
    <div className="flex h-screen bg-neutral text-neutral-foreground overflow-hidden">
            ...
            <a
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-md transition-colors ${
                window.location.pathname.startsWith(item.path) && item.path !== "#"
                  ? "bg-white/20 text-white font-medium border-l-4 border-tertiary" 
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 bg-neutral border-b border-white/10 flex items-center justify-between px-6 shrink-0 md:justify-end">
          <div className="md:hidden flex items-center gap-2 text-primary font-bold text-xl">
             <Building className="h-6 w-6 text-tertiary" /> RentalPro
          </div>
          <div className="flex items-center gap-4">
            ...
            {/* Logout on mobile */}
            <button onClick={handleLogout} className="md:hidden text-slate-400 hover:text-secondary">
              <LogOut className="h-5 w-5" />
            </button>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-auto bg-neutral p-6">
          <Outlet />
        </main>
      </div>
    </div>
